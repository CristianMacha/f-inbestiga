...
export class UserFormComponent implements OnInit, OnDestroy {
  ...
  createUser(): void {
    ...
    this.personService
      .create({
        ...
        personRoles: this.personRolesSelected,
      })
      .subscribe((resp) => {
        this.router.navigateByUrl(`backoffice/user`).then();
        this.loading = false;
  ...
  updateUser(): void {
    ...
    this.personService
      .update({
        ...
        personRoles: this.personRolesSelected,
        user: {
          email: (this.personForm.controls['user'] as FormGroup).controls[
            'email'
          ].value,
        },
      })
  ...
  checkFormStatus(personId: number) {
    this.personService.getByUser(personId).subscribe((resp) => {
      this.editMode = true;
      this.personForm.patchValue(resp);
      (this.personForm.controls['user'] as FormGroup).controls[
        'password'
      ].disable();
      resp.personRoles.forEach((personRole) =>
        this.personRolesSelected.push(personRole)
      );
    });
  }
  ...
  remove(personRole: PersonRoles): void {
    const index = this.personRolesSelected.findIndex(
      (pr) => pr.id == personRole.id
    );
    if (this.editMode && personRole.id !== 0) {
      this.personRolesSelected[index].active =
        !this.personRolesSelected[index].active;
    } else {
      if (index >= 0) {
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    ...
    const index = this.personRolesSelected.findIndex(
      (personRole) => personRole.role.id == newRole.id
    );
    if (index < 0) {
